...
lecture3Spec = describe "Lecture 3" $ do
  ...
  describe "apply" $ do
    it "Maybe int addition" $ apply 5 (Just (+ 3)) `shouldBe` Just 8
    it "Maybe Nothing" $ apply 5 Nothing `shouldBe` (Nothing :: Maybe Int)
    it "Maybe list drop" $ apply [1 .. 10] (Just (drop 7)) `shouldBe` Just [8, 9, 10]
    it "List int operations" $ apply 5 [(+ 3), (* 4), div 17] `shouldBe` [8, 20, 3]
