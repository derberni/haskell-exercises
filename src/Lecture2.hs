...
dragonFight knight dragon = go 0 knight dragon
  where
    go :: Int -> Knight -> Dragon -> Outcome
    go 10 knight dragon = go 0 (knight {knightHealth = knightHealth knight - dragonFirePower dragon}) dragon
    go round knight dragon
      | knightHealth knight < 0 = Loose
      | dragonHealth dragon < 0 = Win
      | knightEndurance knight == 0 = Run
      | otherwise = go (round + 1) (knight {knightEndurance = knightEndurance knight - 1}) (dragon {dragonHealth = dragonHealth dragon - knightAttack knight})
...
claimRewards :: Dragon -> Chest a -> Reward a
claimRewards MkDragon {dragonColor = Red} (MkChest gold treasure) = MkReward 100 gold (Just treasure)
claimRewards MkDragon {dragonColor = Black} (MkChest gold treasure) = MkReward 150 gold (Just treasure)
claimRewards MkDragon {dragonColor = Green} (MkChest gold _) = MkReward 250 gold Nothing
...
-- 10 + 15
-- Add (Lit 10) (Lit 15) -> Lit 25
-- x + 10 + 15
-- Add (Var x) (Add (Lit 10) (Lit 15)) -> Add (Var x) (Lit 25)
-- x + 10 + y + 15
-- Add (Var x) (Add (Lit 10) (Add (Var y) (Lit 15))) -> Add (Var x) (Add (Var y) (Lit 25))
--
-- Add (Add (Var "x") (Lit 7)) (Add (Var "y") (Lit 2))
-- Add (Var "x") (Add (Lit 7) (Add (Var "y") (Lit 2)))
--
constantFolding :: Expr -> Expr
-- match for any 0 literals and remove them, note that these cases also take care of later recursive
-- calls where the sum of two literals evaluates to 0
constantFolding (Add (Lit 0) r) = constantFolding r
constantFolding (Add l (Lit 0)) = constantFolding l
-- match for basic addition of two literals
constantFolding (Add (Lit i) (Lit j)) = Lit (i + j)
-- match cases where two literals can be summed
constantFolding (Add (Lit i) (Add (Lit j) r)) = constantFolding $ Add (Lit (i + j)) r
constantFolding (Add (Lit i) (Add l (Lit j))) = constantFolding $ Add (Lit (i + j)) l
constantFolding (Add (Add (Lit i) r) (Lit j)) = constantFolding $ Add (Lit (i + j)) r
constantFolding (Add (Add l (Lit i)) (Lit j)) = constantFolding $ Add (Lit (i + j)) l
-- use associative property of addition to rearrange the Expr from (a + b) + (c + d)
-- to a + (b + (c + d)). In the latter form, the cases above can then match
constantFolding (Add (Add ll lr) r) = constantFolding $ Add ll (constantFolding (Add lr r))
-- match all other cases, like Var, Add Var Var etc.
constantFolding expr = expr